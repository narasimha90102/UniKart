...
import { ShoppingCart, Heart, Search, Menu, Bell, User, MessageCircle, X, Settings, LogOut, Package, LayoutGrid } from 'lucide-react';
    ...
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isAuthPage ? 'bg-transparent border-transparent' : 'bg-white border-b border-gray-100'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 lg:h-16">
          ...
          {/* ... existing search and right actions ... */}
          {!isAuthPage && (
              ...
              <>
                ...
                <div className="relative group hidden sm:block ml-2">
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-100 shadow-xl rounded-xl py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                    ...
                    <Link to="/dashboard" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Overview</Link>
                    ...
                    <Link to="/sell" className="block px-4 py-2 text-sm text-[#1B8C50] font-medium hover:bg-green-50">Sell Products</Link>
                  </div>
                </div>
              </>
            ) : (
                ...
                {user ? (
                  ...
                  <>
                    <div className="px-2 space-y-3">
                      <Link to="/login" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center justify-center w-full py-4 rounded-2xl bg-gray-50 text-gray-700 font-bold border border-gray-100 active:scale-95 transition-all">
                        Login to Account
                      </Link>
                      <Link to="/signup" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center justify-center w-full py-4 rounded-2xl bg-primary text-white font-bold shadow-lg shadow-primary/20 active:scale-95 transition-all">
                        Create New Account
                      </Link>
                    </div>
                  </>
